...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401) {
      // Check if user is actually logged in before clearing data
      const token = localStorage.getItem('token');
      const user = localStorage.getItem('user');
      
      if (token && user) {
        // Temporarily disable token refresh due to 404 endpoint issue
        console.warn('401 error but user has valid data, not clearing auth');
        return Promise.reject(error);
      } else {
        // Only clear if we don't have valid data
...
export const pipelineAPI = {
  ...
  getAll: async () => {
    try {
      console.log('🔄 Pipeline API: Attempting to fetch from remote backend...');
      const response = await api.get('/pipeline/all');
      console.log('✅ Pipeline API: Successfully fetched from remote backend');
      return response;
    } catch (error) {
      console.error('❌ Pipeline API: Remote backend failed, error:', error.message);
      // Try local backend as fallback
      try {
        console.log('🔄 Pipeline API: Trying local backend fallback...');
        const localApi = axios.create({
          baseURL: 'http://localhost:5000/api',
          headers: { 'Content-Type': 'application/json' }
        });
        const localResponse = await localApi.get('/pipeline/all');
        console.log('✅ Pipeline API: Successfully fetched from local backend');
        return localResponse;
      } catch (localError) {
        console.error('❌ Pipeline API: Both remote and local backends failed');
